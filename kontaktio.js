(function () {
  ...
  scripts.forEach((script, idx) => {
    ...
    const fetchConfig = async () => {
      const cacheKey = `kontaktio-cfg-${CLIENT_ID}`;
      ...
      try {
        ...
        if (cached) {
          ...
          if (Date.now() - ts < CONFIG_TTL) return normalizeClient(data);
        }
      } catch {}
    ...
    const buildMessageRow = (role, text) => {
    ...
    const renderMessage = (role, text) => {
      ...
      wrap.appendChild(buildMessageRow(role, text));
      scrollToBottom();
    };
    ...
    const mount = async () => {
      ...
      if (history.length) {
        ...
        if (wrap) {
          ...
          history.forEach((m) => {
            if (!m || !m.role) return;
            frag.appendChild(buildMessageRow(m.role === "user" ? "user" : "bot", m.text || ""));
          });
